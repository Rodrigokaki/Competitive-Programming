...
nonVisited = set(range(len(colors)))

while nonVisited:
    i = nonVisited.pop()
    ans = 0
    queue = []
    queue.append((i, defaultdict(int), set()))
    while queue:
        cur, colorsDict, curSeen = queue.pop()
        if cur in nonVisited:
            nonVisited.remove(cur)
        colorsDict[colors[cur]] += 1
        ...
        for edge in graph[cur]:
            ...
            curSeen.add(edge)
            queue.append((edge, colorsDict.copy(), curSeen.copy()))
